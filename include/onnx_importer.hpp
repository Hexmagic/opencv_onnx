...
void ONNXImporter::populateNet()
{
    ...
    for (int li = 0; li < layersSize; li++)
    {
        ...
        try{
            ...
            CV_LOG_ERROR(NULL, "EROOR INDEX: "<<li);
        }
...
void ONNXImporter::handleNode(const opencv_onnx::NodeProto &node_proto)
{
    ...
    try
    {
        ...
        {
            CALL_MEMBER_FN(*this, iter->second)
            (layerParams, node_proto);
        }
        else
        {
    ...
    if (layerParams.has("output_shape"))
    {
        ...
        if (layerParams.has("pad_mode"))
        {
            ...
            for (int i = 0; i < strides.size(); i++)
            {
                ...
                adjust_pads.push_back(padMode == "SAME" ? (sz - 1) % stride : (sz - kernel.get<int>(i)) % stride);
            }
...
void ONNXImporter::parseCast(LayerParams &layerParams, const opencv_onnx::NodeProto &node_proto)
{
    if (constBlobs.find(node_proto.input(0)) != constBlobs.end())
    {
        ...
        case opencv_onnx::TensorProto_DataType_FLOAT:
            type = CV_32F;
            break;
        case opencv_onnx::TensorProto_DataType_UINT8:
            type = CV_8U;
            break;
        case opencv_onnx::TensorProto_DataType_UINT16:
            type = CV_16U;
            break;
        case opencv_onnx::TensorProto_DataType_FLOAT16:
            type = CV_16S;
            break;
        ...
        case opencv_onnx::TensorProto_DataType_INT64:
            type = CV_32S;
            break;
        default:
            type = blob.type();
...
void ONNXImporter::parseResize(LayerParams &layerParams, const opencv_onnx::NodeProto &node_proto)
{
    ...
    if (layerParams.has("coordinate_transformation_mode"))
    {
        ...
        {
            layerParams.set("mode", interp_mode == "pytorch_half_pixel" || interp_mode == "half_pixel" ? "opencv_linear" : "bilinear");
        }
    }
...
void ONNXImporter::parseUpsample(LayerParams &layerParams, const opencv_onnx::NodeProto &node_proto)
{
    ...
    if (layerParams.has("coordinate_transformation_mode"))
    {
        ...
        if (layerParams.get<String>("mode") == "linear")
        {
            layerParams.set("mode", interp_mode == "pytorch_half_pixel" ? "opencv_linear" : "bilinear");
        }
    }
    if (layerParams.get<String>("mode") == "linear" && framework_name == "pytorch")
        layerParams.set("mode", "opencv_linear");
